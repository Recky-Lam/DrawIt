...
#define DATE_COMPONENTS ( NSCalendarUnitEra| NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday |  NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond | NSCalendarUnitWeekdayOrdinal)
...
+ (NSString *)dateFormatToString:(NSDate *)date
{
    ...
    if ([[date formatYMD] isEqualToString:[current formatYMD]]) {
        return [date formatHM];
    }else {
        NSString *dateStyle = (current.year != date.year)?@"yyyy-MM-dd":@"MM-dd HH:mm";
        return [NSDate formatDate:date dateStyle:dateStyle];
    }
}
...
- (NSString *)formatMDHM
{
    return [self formatWithStyle:@"MM.dd HH:mm"];
}
...
+ (NSDate *) dateTomorrowAndTomorrow
{
...
- (BOOL)isTodayEastEightArea
{
    NSDate *nowDate = [NSDate dateWithHoursFromNow:8];
    return [self isEqualToDateIgnoringTime:nowDate];
}
...
- (BOOL) isTomorrowAndTomorrow
{
    return [self isEqualToDateIgnoringTime:[NSDate dateTomorrowAndTomorrow]];
}
...
- (NSDate *)setYear:(NSInteger)year
{
    ...
    NSDateComponents *dateComponents = [calendar components:(NSCalendarUnitYear |NSCalendarUnitMonth |NSCalendarUnitDay |NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond) fromDate:self];
